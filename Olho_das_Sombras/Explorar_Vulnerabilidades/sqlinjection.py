...
⠰⠏⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀''')
    ...
    while True:
        print(
            "\n[1] Detectar se o campo é vulnerável a SQL Injection"
            "\n[2] Descobrir se a query usa aspas simples, duplas ou valor numérico"
            "\n[3] Testar se dá pra burlar login ou forçar condição TRUE"
            "\n[4] Descobrir como cortar o resto da query com comentários"
            "\n[5] Ver se a página reage a TRUE e FALSE (testes lógicos)"
            "\n[6] Identificar tipo de banco e sintaxe suportada"
            "\n[7] Confirmar SQL Injection cega com testes AND"
            "\n[8] Descobrir quantas colunas a query possui (ORDER BY)"
            "\n[9] Testar colunas usando agregações (alternativa ao ORDER BY)"
            "\n[10] Extrair dados usando UNION SELECT"
            "\n[11] Testar uso de subconsultas (subqueries)"
            "\n[12] Descobrir versão e informações do banco"
            "\n[13] Enumerar tabelas em banco SQL Server (MSSQL)"
            "\n[14] Confirmar SQL Injection cega usando atraso (Time-Based)"
        )
